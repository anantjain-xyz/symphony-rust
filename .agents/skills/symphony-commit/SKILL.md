...
3. Sanity-check newly staged files. Build artifacts, logs, screenshots, `.symphony-workspace-ready`, and locally injected fallback skills under `.agents/skills/symphony-*` or `.claude/skills` must stay unstaged unless the issue explicitly asks to install or change Symphony skills. Exception: `.symphony/screenshots/<name>.png` is intentionally staged only by the `symphony-screenshot` skill's temporary evidence commit (which also `--no-verify`s, the only place that's allowed); that skill then creates a cleanup revert commit so the files do not remain in the latest tree while the evidence commit remains in branch history.
