...
- **Linear**: `$LINEAR_API_KEY` is exported into your environment by Symphony. If Linear MCP tools are configured for the run, use their advertised operations regardless of the MCP server name; otherwise call the HTTP API directly: `curl -fsS -H "Authorization: $LINEAR_API_KEY" -H "Content-Type: application/json" https://api.linear.app/graphql -d '{"query":"..."}'`. Do not spend turns guessing tool namespaces — the HTTP path always works.
