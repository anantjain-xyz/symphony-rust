...
tracker:
  kind: linear
  # Stored in the OS keychain; populated by Symphony at runtime.
  ...
  # session pages. Populated from Settings — leave the placeholder as-is.
  workspace: ${SYMPHONY_LINEAR_WORKSPACE}
  # Restrict the worker to one team in the workspace by issue-identifier prefix
  # (e.g. "SYM-"). Required when the API key has access to multiple teams in
  # one workspace. Populated from Settings; if unset, no filter is applied.
  ...
  # may be set, in which case the filters intersect. Populated from Settings;
  # if unset, no project filter is applied.
  project_id: ${SYMPHONY_TRACKER_PROJECT_ID}
  # State names match Linear's workflow states case-insensitively.
...
workspace:
  # Populated from Settings ("Workspace root"); falls back to the app data dir.
  root: ${TMPDIR}

hooks:
  # Runs once per fresh workspace. $REPO_URL, $ISSUE_IDENTIFIER, and
  # $ISSUE_BRANCH are provided by Symphony.
  after_create: |
    git clone "$REPO_URL" .
...
agent:
  # codex | claude — populated from Settings.
...
codex:
  ...
  # The workflow pushes branches and talks to GitHub/Linear — keep network on.
  network_access: true
...
claude:
  ...
  allowed_tools:
    ...
    # Fetching canonical boilerplate docs (LICENSE, CODE_OF_CONDUCT, ...) and
    # the Linear GraphQL API — see Environment and Guardrails.
    - Bash(curl *)
  disallowed_tools: []
  add_dirs: []
  turn_timeout_ms: 3600000
...
Repeatable mechanics live under `.agents/skills/<name>/SKILL.md` in the target repo (the canonical, runner-agnostic location; `.claude/skills/<name>` are symlinks Claude Code uses for auto-discovery). Reach for them by name — your runner will surface the right one on demand:
...
- **GitHub**: the `gh` CLI inherits the host's authentication. If `gh pr edit --add-label` 500s (a known Projects-classic GraphQL deprecation), apply labels via the REST API instead — the `push` skill covers this.
...
2. Run required validation for the scope.
   ...
   - User-facing → exercise the path locally and capture comprehensive evidence via the `screenshot` skill: full-page Playwright captures of every state worth reviewing (happy path, loading, error, mobile, hover) embedded in the PR description. Logs/CLI output supplement screenshots, they do not replace them.
